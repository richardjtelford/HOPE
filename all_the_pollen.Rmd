...
allPollen %>% filter(species == "Lycopodium", count > 1000) %>% count(.id)

allPollen %>% filter(species == "Lycopodium") %>% ggplot(aes(x = count)) + geom_histogram() + xlim(0, 1000)

```

Some of these are probably easy to explain. 4355 is either in the middle of the densest stand of _Lycopodium_ since the Carboniferous, or the Lycopodium spike has been mis-labelled.
